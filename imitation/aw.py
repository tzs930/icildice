...
class AdvWBC(nn.Module):
    def __init__(self, policy, env, best_policy=None,
                 ...
                 w_hidden_size=256, inner_steps=10, gamma=0.99, temperature=1e-3,
        ...
        self.w_network = nn.Sequential(
            nn.Linear(self.obs_dim+self.action_dim, self.w_hidden_size, device=self.device),
            # nn.LeakyReLU(0.2, inplace=True),
            nn.ReLU(inplace=True),
            nn.Linear(self.w_hidden_size, 1, device=self.device),
            nn.ELU(inplace=True)
        )
        ...
        self.w_optimizer = optim.Adam(self.w_network.parameters(), lr=lr)
    ...
    def train(self, total_iteration=1e6, eval_freq=1000, batch_size=1024):
        ...
        for num in range(0, int(total_iteration)+1):
            for _ in range(self.inner_steps):
                ...
                # w training; maximize weighted cost
                C_pi_s = ((actions - self.policy(obs).mean)**2).mean(dim=-1)
                w_s_a = self.w_network( torch.cat((obs, actions), dim=-1)).reshape(-1) + 1
                w_loss = - (C_pi_s.detach() * w_s_a).mean()
            ...
            if self.weighted_replay_sampling:
                ...
                C_pi_s = ((actions - self.policy(obs).mean)**2).mean(dim=-1)
                full_weights = self.w_network( torch.cat((obs, actions), dim=-1)).reshape(-1) + 1
                ...
                # if self.weight_norm:
                normalized_w_e = w_s_a / w_s_a.mean()
                policy_loss = (normalized_w_e.detach() * C_pi_s).mean()
                # else:
                #     policy_loss = (w_s_a.detach() * C_pi_s).mean()
            ...
            if (num) % eval_freq == 0:
                ...
                w_s_a_valid = self.w_network(torch.cat((obs_valid, actions_valid), dim=-1)).reshape(-1) + 1
                
                policy_loss_valid = (w_s_a_valid.detach() * C_pi_s_valid).mean()
                ...
                w_train_avg = w_s_a.mean()
                ess_train = (w_s_a.sum()) ** 2 / (w_s_a **2 ).sum()
                
                w_valid_avg = w_s_a_valid.mean()
                ess_valid = (w_s_a_valid.sum()) ** 2 / (w_s_a **2 ).sum()
